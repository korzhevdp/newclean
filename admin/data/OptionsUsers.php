...
<div class="table header">
	<div class="table-row">
		...
		<div style="width: 14%;" data-field="email" data-required>E-mail / логин</div>
		<div style="width: 11%;">Посещение</div>
		<div style="width: 15%;" data-field="alias" data-required>Имя пользователя</div>
		<div style="width: 7%;  text-align: center;">Кол-во сообщений</div>
...
foreach($arUsers as $key => $user)
{
	...
	echo "<div class='table-row' data-row-id='".$key."' style='".$color."'>";
		echo "<div style='width: 4%'><a href='#' class='info searchable'>".$key."</a></div>";
		echo "<div style='width: 14%'><a href='#' class='info searchable' title='".$user['PHONE']."'>".($user['EMAIL']!='' ? $user['EMAIL']: '')."</a><input type='text' name='email'/></div>";
		echo "<div style='width: 11%'><a href='#' class='info'>".($user['AUTH_DATE']!='' ? $user['AUTH_DATE']: '')."</a></div>";
