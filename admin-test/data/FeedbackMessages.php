...
<div class="table header">
	<div class="table-row">
		...
		<div style="width: 15%;">Скриншот</div>
		<div style="width: 10%;">Время отправки</div>
		<div style="width: 10%; text-align: center;" data-field="answered">
			<span>Отвечено</span>
...
foreach($arMessages as $key => $message)
{
	...
	echo "<div class='table-row' data-row-id='".$key."' style='".$color."'>";
		...
		echo "<div style='width: 15%'><a href='#' class='info searchable'>".($message['file_path']!='' ? '<img style="max-width: 200px;" src="/'.$message['file_path'].'">': '')."</a></div>";
		echo "<div style='width: 10%'><a href='#' class='info'>".($message['create_date']!='' ? $message['create_date']: '')."</a></div>";
		echo "<div style='width: 10%; text-align: center;'><a href='#answered_select' class='info' data-id='".$message['answered']."'>".($message['answered']== 1 ? "Да":"Нет")."</a><input type='hidden' name='answered' value='".$message['answered']."'/></div>";
	echo "</div>";
}
