...
if(!$law7)
{
    $_SESSION['SSUID'] = null;
    $_COOKIE['SSUID'] = null;
    exit('<div class="big-logo"><div class="img"></div></div><p class="page-error">Извините, у вас недостаточно прав для доступа к данному разделу системы.<a href="#" id="reload" onclick="window.location.reload();" class="icon-left-open-big">Вернуться обратно</a></p>');
}
...
if($law2 || $law1 || $law6)
{
	...
	else
	{
		if($UserOrganization)
		{
			$arMessages = Messages::GetMessages($_SESSION['UID'],6,0,$DepartId);
		}
		else
		{
